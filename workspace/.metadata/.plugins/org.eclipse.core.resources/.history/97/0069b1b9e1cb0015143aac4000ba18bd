...
/*
 * 바로 run 메소드를 실행하면, jvm 이 메인쓰레드를 생성해 프로그램을 실행함.
 * 그러므로, 쓰레드는 main 이라는 이름으로 쓰레드가 실행하며, run 메소드를 차래대로 실행된다.
 * st.getState() 메소드를 실행해 보면, st 쓰레드(첫번째)는 new 인 상태로 존재한다.
 * st.start로 실행하면, "첫번째"라는 이름으로 쓰레드를 만들고 Runnable 상태로 변경하고,
 * 쓰레드가 정상적으로 작동한다.
 * 
 * 주의할점은 메인메소드도 쓰레드로 동작하므로, st.start 나 st.run 이나 동작하는 방법은
 * 다르지 않다.
 * 
